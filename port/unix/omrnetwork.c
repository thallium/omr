...
uintptr_t
network_get_network_interfaces(struct OMRPortLibrary *portLibrary, OMRNetworkInterfaceCallback callback, void *userData)
{
	...
	if (NULL != procNetDevFile) {
		...
		for (currentInterface = interfaceList; NULL != currentInterface; currentInterface = currentInterface->ifa_next) {
			...
			callbackResult = callback(currentInterface->ifa_name, 0, 0, userData);
			if (0 != callbackResult) {
				break;
			}
		}
